...
buttons.forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        // todo: consertar a busca de dados da presença
        ...
        if (presencas.success && presencas.result) {
            ...
            if (presencasElement) presencasElement.textContent = `Presenças: ${presencas.result.presencas}`;
            if (faltasElement) faltasElement.textContent = `Faltas: ${presencas.result.faltas}`;
        }
    });
});
