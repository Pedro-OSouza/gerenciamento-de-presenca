<?php
    ...
    include_once __DIR__.'/scripts/php/services/presenca_services.php';

    $id_aluno = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);

    $presenca = new Presenca_services();
    // posteriormente aluno deve consumir uma lógica de services
    $aluno = new Aluno();
    $dados_aluno = $aluno->buscarPorId($id_aluno) ?? [];
    $presencas_aluno = $presenca->buscarHistoricoPresencas($id_aluno) ?? [];
    $total_presencas = $presenca->buscarPresencasAcumuladas($id_aluno) ?? 0;
    $total_faltas = $presenca->buscarFaltasAcumuladas($id_aluno) ?? 0;
    $repo_devidas = $presenca->bucarReposicoesDevidas($id_aluno) ?? 0;
    $repo_feitas = $presenca->buscarReposicoesFeitas($id_aluno);
