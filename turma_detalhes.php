...
$aula = new Aula();
$aulas = $aula->buscarPorTurma($turma_id);
$aula_atual = !empty($aulas) ? $aulas[0]['id'] : null;
    ...
    <main class="section" style="padding-top: 6rem;">
        <div class="container">
            <div id="turma-info" class="is-flex is-justify-content-space-between is-align-items-center mb-4">
                <div>
                    <h2 class="title has-text-white is-4">Turma: <?= ucfirst(htmlspecialchars($dados_turma['nome'])) ?></h2>
            ...
            <div class="box has-background-dark has-text-white">
                ...
                <input type="hidden" id="aula-id" value="<?= $aula_atual ?>">
                ...
                <?php foreach ($alunos as $aluno): ?>
                        <div class="columns is-vcentered is-mobile" style="border-bottom: 1px solid rgba(255,255,255,0.1); border-radius: 0;">
                            <div class="column is-6">
                                    <p class="has-text-weight-semibold has-text-white">
                                        <?= htmlspecialchars($aluno['aluno_nome']) ?>
                                    </p>
                                    <p class="is-size-7 has-text-grey-light">Presenças:
                                    <?= htmlspecialchars($aluno['total_presencas']) ?>
                                    </p>
                                    <p class="is-size-7 has-text-grey-light">Faltas: <?= htmlspecialchars($aluno['total_faltas']) ?></p>
                            </div>
                            ...
                            <div class="column is-6 has-text-right">
                                ...
                                <div class="buttons are-small is-right presenca-actions">
                                    <div class="btn-presenca button is-success is-light " data-aluno-id="<?= $aluno['aluno_id'] ?>" data-presente="1">
                                        ✅Presente
                                    </div>
                                    <div class="btn-presenca button is-danger is-light" data-aluno-id="<?= $aluno['aluno_id'] ?>" data-presente="0">
                                        ❌Ausente
                                    </div>
                                </div>
                            </div>
